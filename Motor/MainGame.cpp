...
void MainGame::handleInput()
{
	...
	//Al presionar f, se imprime en consola la cantidad de humanos y zombies
	if (inputManager.isKeyPressed(SDLK_f)) {
		cout << "\nTotal de Humanos: " << TotalHumanos;
		cout << "\nTotal de zombies: " << TotalZombies << endl;
	}
...
void MainGame::initLevel() {
	...
	currentLevel = 0;
	//inicializamos las variables TotalHumanos y TotalZombies
	// 
	TotalHumanos = levels[currentLevel]->getNumHumans();
	TotalZombies = levels[currentLevel]->getZombiesPosition().size();
	...
	for (size_t i = 0; i < zombies.size(); i++) {
		...
		for (size_t j = 0; j < humans.size(); j++)
		{
			if (zombies[i]->collideWithAgent(humans[j])) {
				...
				//Actualizamos el num de zombies y humanos
				TotalZombies++;
				TotalHumanos--;
			}
		}
